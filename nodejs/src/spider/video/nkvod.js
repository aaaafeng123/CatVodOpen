...
async function home(inReq, _outResp) {
    const classes = [{'type_id':'1','type_name':'电影'},{'type_id':'2','type_name':'电视剧'},{'type_id':'3','type_name':'综艺'},{'type_id':'4','type_name':'动漫'},{'type_id':'5','type_name':'动画片'}];
    /*
    const filterObj = {
...
async function category(inReq, _outResp) {
    ...
    const limit = 72;
    const hasMore = $('div.btn-group > a:contains(下一页)').length > 0;
    ...
    for (const id of ids) {
        ...
        try {
            vod = {
                ...
                vod_type: $('div.card-body br:first')[0].next.data.trim().replace('类型：',''),
                vod_year: $('div.card-body br')[2].next.data.trim().replace('年份：',''),
                vod_area: $('div.card-body br')[3].next.data.trim().replace('地区：',''),
                vod_actor: $('div.card-body br')[6].next.data.trim().replace('主演：',''), //$('div.card-body:contains(主演：)').text().trim().substring(3).replace(/\/$/, ''),
                vod_director: $('div.card-body br')[4].next.data.trim().replace('导演：',''), //$('.module-info-item:contains(导演：)').text().trim().substring(3).replace(/\/$/, ''),
                vod_pic: $('.module-info-poster img:first').attr('data-original'),
                vod_remarks : $('.module-info-item:contains(备注：)').text(),
                vod_content: $('p.card-text').find('span').text().replace('简介：',''), // $('.module-info-introduction-content').text().trim(),
            }
        } catch (error) {
            console.log()
        }
